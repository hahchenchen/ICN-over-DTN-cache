...
static void *       recvBundles(void *args)
{
	...
	while(running)
	{
		...
		while(bundleLenRemaining > 0)
		{
			...
			int file_id = iopen("/home/istin/dtn_to_ccn_file",O_RDWR|O_CREAT|O_TRUNC, 0777);
			if(file_id == -1)
			{
			...
			sendto_num=sendto(sockfd, (char*)&data, 1, 0, (struct sockaddr *)(&src_addr), sizeof(src_addr));
			if(sendto_num == 0)
			{
