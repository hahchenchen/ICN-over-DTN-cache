...
int file_fd = open("/home/istin/dtn_to_ccn_file", O_RDWR|O_CREAT|O_TRUNC,
	...
	DEBUGMSG(DEBUG, "write succeed\n");
	    ...
	    /* 打开文件 */
    FILE *fp = fopen("/home/istin/dtn_to_ccn_file", "r"); 
    if(NULL == fp) 
    { 
      printf("File:%s Not Found.\n", "/home/istin/dtn_to_ccn_file"); 
    } 
    else
    { 
      int len = 0; 
        ...
        for (i = 0; i < ccnl->ifcount; i++) {
            if (FD_ISSET(ccnl->ifs[i].sock, &readfs)) {
                ...
                if ((len = recvfrom(ccnl->ifs[i].sock, buf, sizeof(buf), 0,
                    ...
                    if (0) {}
#ifdef USE_IPV4
                    else if (src_addr.sa.sa_family == AF_INET && len>0) {
...
#endif
#ifdef USE_UNIXSOCKET
                    else if (src_addr.sa.sa_family == AF_UNIX && len ==1) {
			DEBUGMSG(INFO, "len=%d\n",len);
			int file_id = open("/home/istin/dtn_to_ccn_file",O_RDONLY);
			len = read(file_id, buf, sizeof(buf));
			if(len<0)
			  DEBUGMSG(ERROR, "could not read dtn_to_ccn_file\n");
			DEBUGMSG(INFO, "dtn_to_ccn_file len is: %d\n", len);
			close(file_id);			
